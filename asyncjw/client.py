...
class Client:
    ...
    def certifications(self):
        return {
            key: list(value.values()) for key, value in self._certifications.items()
        }
    ...
    async def _initialize(self):
        ...
        self._certifications["movie"] = {
            c["technical_name"]: Certification(self, c) for c in data
        }

        data = await self.http.get_certifications("show")
        self._certifications["show"] = {
            c["technical_name"]: Certification(self, c) for c in data
        }
    ...
    async def search(self, query=None, **params):
        ...
        filtered = {
            key: get_id(value) for key, value in params.items() if key in payload.keys()
        }
